...
1. `가연` 또는 `소울` 탭을 고릅니다.
2. **순공 타이머**에서 시작/종료를 누르면 오늘 공부시간에 자동으로 누적됩니다.
3. **강의 일정표**에서 현재 보고 있는 섹션과 강의 번호를 저장합니다.
4. **문제풀기** 버튼을 누릅니다.
5. 모달에서 각자 가지고 있는 DOCX 파일을 선택합니다.
   - 한 번 읽은 DOCX는 문제 JSON으로 브라우저에 저장됩니다.
   - 이후 새로고침해도 바로 이어서 풀 수 있습니다.
   - 다른 덤프로 바꾸고 싶으면 **DOCX 선택/교체**를 다시 누르거나 **문제은행 삭제**를 사용합니다.
6. 문제를 풀고 **정답 보기**를 누르면 세션 통계가 쌓입니다.
7. **세션 저장**을 누르면 현재 탭 사용자 기록에 자동 반영됩니다.
8. 요약정리는 섹션을 선택한 뒤 MD/TXT 파일을 올리거나 직접 붙여넣어 저장합니다.
   - 섹션에 안 묶을 요약은 `기타`에 저장합니다.
   - 예전 요약처럼 섹션 정보가 없는 데이터는 `기타`로 표시됩니다.
   - GitHub 토큰이 연결되어 있으면 저장/삭제가 `summary-notes.json`에도 커밋됩니다.

`합계` 탭은 비교 화면이라 문제풀이와 요약정리 입력은 숨겨지고, 강의 일정표에서 둘의 현재 위치를 비교합니다.
